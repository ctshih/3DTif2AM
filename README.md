...
這是一個簡單的 Python 工具，用於將 3D Tiff 影像轉換為 AmiraMesh 3D ASCII 2.0 或 BINARY-LITTLE-ENDIAN 2.1 格式。

## 功能
- **圖形介面**：基於 Streamlit 的瀏覽器介面。
- **支援兩種格式**：可選擇輸出為容易閱讀的 ASCII 或是高效率的 Binary 格式。
...
5. 選擇輸出的 **Format** (ASCII 或 Binary)。
6. 點擊 **Convert to AmiraMesh** 按鈕。
7. 選擇 `.am` 檔案的儲存位置。
8. 等待顯示「Conversion Complete」訊息即完成。
